...
def get_recommend_bucket_list_refresh(uid, df, user_log, category_c_sim, top=30):
    ...
    for log in user_log['rating'] : 
        if (top <= log) :
            quest_index = int(user_log['quest_num'][cnt])
            top = log
        cnt = cnt+1
    ...
    print(df['quest_num'])
    print(type(df['quest_num']))

    print(df['quest_num'] == quest_index)
    target_bucketlist_index = df[df['quest_num'] == quest_index].index.values
    print('target : ')
    print(target_bucketlist_index)
...
if __name__ == "__main__":
    ...
    get_recommend_bucket_list_refresh(uid, data, user_log, category_c_sim)
